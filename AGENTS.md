...
- **连接参数（敏感，不入库）**：SSH 密钥、跳板 IP/端口、转发目标、隧道端口、远程项目
  路径等连接信息统一放在 `configs/cuda-machine.local.yaml`（git 不跟踪，AI 助手需要
  具体值时读它；模板是 `configs/cuda-machine.example.yaml`，`cp` 一份后填写）。
  上游权威配置是 `~/.renew_core/gpu_train.json`（RenewForecast 项目共用）。
...
- **远程项目路径**：GPU 机上为本项目单独建目录（与 RenewForecast 的目录分开，不要混用），
  具体路径见 `configs/cuda-machine.local.yaml` 的 `paths.remote_project_dir`；
  首次使用需在 GPU 机上创建并构建 `.venv`。
- 代码同步：从本机直接 push 到 GPU 机上的 git remote（`cudabox`，地址见
  `configs/cuda-machine.local.yaml` 的 `git.cudabox_remote`，配 `GIT_SSH_COMMAND`
  指定密钥）。远程仓库需先在 GPU 机上 `git init --bare` 或通过 Gitea 建立。
...
- **注意**：跳板机 IP / 端口会变，连不上时先看 `gpu_tunnels.sh show` 并更新
  `~/.renew_core/gpu_train.json` 与 `configs/cuda-machine.local.yaml`。
